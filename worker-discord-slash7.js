...
// ── Registrar comandos ────────────────────────────────────────────────────
// ── Post automático ───────────────────────────────────────────────────────
async function postCanal(canalId, embed, content, env) {
...
async function autoNovosEps(env) {
  const d = await jikan('/seasons/now?limit=25')
  const diaSemana = new Date().getDay()
  const diasIngles = {'Sundays':0,'Mondays':1,'Tuesdays':2,'Wednesdays':3,'Thursdays':4,'Fridays':5,'Saturdays':6}
  const list = (d.data || []).filter(a => diasIngles[a.broadcast?.day] === diaSemana).slice(0, 5)
  if (list.length === 0) {
    const disponiveis = [...new Set((d.data||[]).map(a => a.broadcast?.day).filter(Boolean))].join(', ')
    throw new Error(`Nenhum anime para dia ${diaSemana}. Dias na API: ${disponiveis || 'nenhum'}`)
  }
  await postCanal(CANAIS.novosEps, {
    title: '📺 Novos Episódios Hoje!',
    description: `**${list.length} animes** lançaram episódios hoje!\n[▶ Ver todos](${SITE_URL}/category/airing)`,
    color: 0xE53935,
    thumbnail: { url: imgHD(list[0]) },
    fields: list.map(a => ({
      name: (a.title_english || a.title).slice(0, 50),
      value: `${nota(a.score)}\n[▶ Assistir](${SITE_URL}/watch/${a.mal_id}?ep=1)`,
      inline: true
    })),
    footer: { text: 'Up Anime+ • Assista grátis!' },
    timestamp: new Date().toISOString()
  }, `🎌 **Novos episódios disponíveis!** → ${SITE_URL}`, env)
}

async function autoAnimesAlta(env) {
  const d = await jikan('/top/anime?filter=airing&limit=6')
  const list = (d.data||[]).slice(0, 5)
  if (!list.length) return

  await postCanal(CANAIS.animesAlta, {
    title: '🔥 Animes em Alta esta Semana!',
    color: 0xFFB300,
    thumbnail: {url: imgHD(list[0])},
    fields: list.map((a,i) => ({
      name: `${i+1}. ${(a.title_english||a.title).slice(0,45)}`,
      value: `${nota(a.score)}
[▶](${SITE_URL}/anime/${a.mal_id})`,
      inline: true
    })),
    footer: {text: 'Up Anime+ • Ranking Semanal'},
    timestamp: new Date().toISOString()
  }, '', env)
}

async function autoTopAnimes(env) {
  const d = await jikan('/top/anime?limit=10&filter=bypopularity')
  const list = d.data||[]
  const desc = list.map((a,i) =>
    `**${i+1}.** [${(a.title_english||a.title).slice(0,40)}](${SITE_URL}/anime/${a.mal_id}) ${a.score?`⭐${a.score.toFixed(1)}`:''}`
  ).join('\n')

  await postCanal(CANAIS.topAnimes, {
    title: '⭐ Top 10 Animes da Semana',
    description: desc + `\n\n[▶ Ver todos](${SITE_URL}/category/bypopularity)`,
    color: 0x8E24AA,
    thumbnail: {url: imgHD(list[0])},
    footer: {text: 'Up Anime+ • Atualizado semanalmente'},
    timestamp: new Date().toISOString()
  }, '', env)
}

async function autoCalendario(env) {
  const dias = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday']
  const hoje = dias[new Date().getDay()]
  const d = await jikan(`/schedules?filter=${hoje}&limit=10`)
  const list = (d.data||[]).slice(0, 8)
  if (!list.length) return

  await postCanal(CANAIS.calendario, {
    title: `📅 Calendário de Hoje — ${hoje.charAt(0).toUpperCase()+hoje.slice(1)}`,
    color: 0x1E88E5,
    fields: list.map(a => ({
      name: (a.title_english||a.title).slice(0, 50),
      value: `${traduzir(a.genres)}\n[▶](${SITE_URL}/anime/${a.mal_id})`,
      inline: true
    })),
    footer: {text: 'Up Anime+ • Lançamentos do dia'},
    timestamp: new Date().toISOString()
  }, '', env)
}
...
export default {
  async scheduled(event, env, ctx) {
    ...
    // Novos eps — todo dia às 20h (23h BRT)
    if (hora === 23) ctx.waitUntil(autoNovosEps(env))

    // Calendário — todo dia às 12h (15h BRT)
    if (hora === 15) ctx.waitUntil(autoCalendario(env))

    // Animes em alta — segunda-feira às 20h
    if (hora === 23 && diaSemana === 1) ctx.waitUntil(autoAnimesAlta(env))

    // Top animes — domingo às 20h
    if (hora === 23 && diaSemana === 0) ctx.waitUntil(autoTopAnimes(env))
  },

  async fetch(request, env, ctx) {
    const url = new URL(request.url)
    // Testar posts manualmente — await direto para ver erros
    if (url.searchParams.get('action') === 'test_auto') {
      const tipo = url.searchParams.get('tipo') || 'eps'
      try {
        if (tipo === 'eps')        await autoNovosEps(env)
        if (tipo === 'alta')       await autoAnimesAlta(env)
        if (tipo === 'top')        await autoTopAnimes(env)
        if (tipo === 'calendario') await autoCalendario(env)
        return new Response(`✅ Post enviado: ${tipo}`)
      } catch(e) {
