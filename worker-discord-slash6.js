...
// ── Traduz synopsis EN→PT via MyMemory (gratuito) ─────────────────────────
async function traduzirSinopse(text) {
  if (!text) return '—'
  try {
    const short = text.slice(0, 400)
    const r = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(short)}&langpair=en|pt-br`)
...
async function registerCommands(env) {
  ...
  const cmds = [
    ...
    {name:'ova',          description:'📼 Melhores OVAs'},
    {name:'nota',         description:'⭐ Animes mais bem avaliados'},
    {name:'comparar',     description:'⚔️ Compara dois animes', options:[{type:3,required:true,name:'anime1',description:'Primeiro anime'},{type:3,required:true,name:'anime2',description:'Segundo anime'}]},
    {name:'ep',           description:'📺 Info de um episódio específico', options:[{type:3,required:true,name:'anime',description:'Nome do anime'},{type:4,required:true,name:'numero',description:'Número do episódio'}]},
  ...
  try {
    switch(cmd) {
      ...
      case 'dia': {
        ...
        await editReply(token, {title:`🎬 Anime do Dia: ${t}`,description:await traduzirSinopse((a.synopsis||'').slice(0,400)),color:COR.rosa,url:`${SITE_URL}/anime/${a.mal_id}`,image:{url:imgHD(a)},
      ...
      case 'recomendacao': {
        ...
        if (!a) { await editErr(token,'Sem dados',env); break }
        const t = a.title_english||a.title
        await editReply(token, {title:`🎲 ${cmd==='aleatorio'?'Aleatório':'Recomendação'}: ${t}`,description:await traduzirSinopse((a.synopsis||'').slice(0,400)),color:COR.verde,url:`${SITE_URL}/anime/${a.mal_id}`,image:{url:imgHD(a)},
      ...
      case 'personagem': {
        ...
        await editReply(token, {title:`👤 ${c.name}`,description:await traduzirSinopse((c.about||'').slice(0,300))||'Sem descrição.',color:COR.azul,
      ...
      case 'anime': {
        ...
        await editReply(token, {title:t,description:await traduzirSinopse((a.synopsis||'').slice(0,400)),color:COR.verde,url:`${SITE_URL}/anime/${a.mal_id}`,thumbnail:{url:imgSM(a)},image:{url:imgHD(a)},
      ...
      case 'filme': {
        const d = await jikan('/top/anime?type=movie&limit=8&filter=bypopularity')
        const list = (d.data||[]).slice(0,6)
...
[▶ Assistir](${SITE_URL}/anime/${a.mal_id})`,inline:true})),
          footer:{text:'Up Anime+'},timestamp:new Date().toISOString()}, '', env)
        break
      }

      case 'ova': {
        const d = await jikan('/top/anime?type=ova&limit=8&filter=bypopularity')
        const list = (d.data||[]).slice(0,6)
        await editReply(token, {title:'📼 Melhores OVAs',color:COR.roxo,thumbnail:{url:imgHD(list[0])},
          fields:list.map((a,i)=>({name:`${i+1}. ${(a.title_english||a.title).slice(0,45)}`,value:`${nota(a.score)}
[▶](${SITE_URL}/anime/${a.mal_id})`,inline:true})),
          footer:{text:'Up Anime+'},timestamp:new Date().toISOString()}, '', env)
        break
      ...
      case 'nota': {
        const d = await jikan('/top/anime?limit=8&filter=favorite')
        const list = (d.data||[]).slice(0,6)
      ...
      case 'comparar': {
        const [d1, d2] = await Promise.all([
          jikan(`/anime?q=${encodeURIComponent(opts.anime1||'')}&limit=1&sfw=true`),
          jikan(`/anime?q=${encodeURIComponent(opts.anime2||'')}&limit=1&sfw=true`),
        ])
        ...
        await editReply(token, {title:`⚔️ ${t1} vs ${t2}`,color:COR.vermelho,
          fields:[
            {name:`🎌 ${t1}`,value:`${nota(a1.score)}\n📺 ${a1.episodes||'?'} eps\n${traduzir(a1.genres)}\n[▶ Assistir](${SITE_URL}/anime/${a1.mal_id})`,inline:true},
            {name:'VS',value:'⚔️',inline:true},
            {name:`🎌 ${t2}`,value:`${nota(a2.score)}\n📺 ${a2.episodes||'?'} eps\n${traduzir(a2.genres)}\n[▶ Assistir](${SITE_URL}/anime/${a2.mal_id})`,inline:true},
            {name:'🏆 Vencedor',value:(a1.score||0)>=(a2.score||0)?`**${t1}** com ${nota(a1.score)}`:`**${t2}** com ${nota(a2.score)}`,inline:false},
          ],
          footer:{text:'Up Anime+'},timestamp:new Date().toISOString()}, '', env)
        break
      }
      ...
      case 'ep': {
        const d = await jikan(`/anime?q=${encodeURIComponent(opts.anime||'')}&limit=1&sfw=true`)
        const a = d.data?.[0]
        if (!a) { await editErr(token,`"${opts.anime}" não encontrado.`,env); break }
        const epNum = opts.numero||1
        const epData = await jikan(`/anime/${a.mal_id}/episodes/${epNum}`)
        const ep = epData.data
        const titulo = a.title_english||a.title
        await editReply(token, {title:`📺 ${titulo} — EP ${epNum}`,
          description:ep?.synopsis ? await traduzirSinopse(ep.synopsis) : 'Sem descrição disponível.',
          color:COR.azul,url:`${SITE_URL}/watch/${a.mal_id}?ep=${epNum}`,
          thumbnail:{url:imgSM(a)},
          fields:[
            {name:'🎬 Título do EP',value:ep?.title||`Episódio ${epNum}`,inline:true},
            {name:'▶ Assistir',value:`[Clique aqui!](${SITE_URL}/watch/${a.mal_id}?ep=${epNum})`,inline:true},
          ],
          footer:{text:'Up Anime+'},timestamp:new Date().toISOString()}, '', env)
        break
      }
