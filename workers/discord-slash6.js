...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // Testar posts manualmente
    if (url.searchParams.get('action') === 'test_auto') {
      const tipo = url.searchParams.get('tipo') || 'eps'
      if (tipo === 'eps')      ctx.waitUntil(autoNovosEps(env))
      if (tipo === 'alta')     ctx.waitUntil(autoAnimesAlta(env))
      if (tipo === 'top')      ctx.waitUntil(autoTopAnimes(env))
      if (tipo === 'calendario') ctx.waitUntil(autoCalendario(env))
      return new Response(`✅ Testando post: ${tipo}`)
    }
