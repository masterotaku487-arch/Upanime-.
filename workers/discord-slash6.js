...
async function processCommand(cmd, opts, token, body, env) {
  try {
    ...
    // ── ADM guard ──────────────────────────────────────────────────────────
    const admCmds = ['anuncio','stats','banir','dc','setup','criar_cargo','criar_canal']
    ...
    switch(cmd) {
      ...
      case 'setup': {
        ...
        const cargos = [
          ...
          {name:'Moderador', color:0xE53935, hoist:true},
        ...
        // Categorias + canais
        const struct = [
          {cat:'INFORMACOES', canais:[{name:'bem-vindo',type:0},{name:'regras',type:0},{name:'anuncios',type:5}]},
          {cat:'ANIME',       canais:[{name:'geral-anime',type:0},{name:'lancamentos',type:0},{name:'recomendacoes',type:0}]},
          {cat:'COMUNIDADE',  canais:[{name:'geral',type:0},{name:'bot-comandos',type:0},{name:'voz-geral',type:2}]},
          {cat:'STAFF',       canais:[{name:'mod-chat',type:0},{name:'logs',type:0}]},
        ]
        let totalCanais = 0
        for (const {cat, canais} of struct) {
          const cr = await fetch(`https://discord.com/api/v10/guilds/${gId}/channels`,{method:'POST',headers:h,body:JSON.stringify({name:cat,type:4})})
          const catData = await cr.json()
          await wait(400)
          for (const canal of canais) {
            await fetch(`https://discord.com/api/v10/guilds/${gId}/channels`,{method:'POST',headers:h,body:JSON.stringify({...canal,parent_id:catData.id})})
            totalCanais++
            await wait(350)
          }
        }
        ...
        await fetch(`https://discord.com/api/v10/webhooks/${CLIENT_ID}/${token}/messages/@original`,{
          ...
          body: JSON.stringify({embeds:[{
            ...
            fields:[
              {name:'🎭 Cargos',    value:`${cargos.length} cargos criados`, inline:true},
              {name:'📁 Categorias',value:'4 categorias criadas',             inline:true},
              {name:'💬 Canais',    value:`${totalCanais} canais criados`,    inline:true},
              {name:'Cargos',value:'Dono • Moderador • VIP • Otaku • Novato', inline:false},
            ],
            footer:{text:'Up Anime+ ADM'}
          }]})
        })
        break
      }
