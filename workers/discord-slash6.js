...
async function autoNovosEps(env) {
  const d = await jikan('/seasons/now?limit=10')
  const hoje = new Date()
  const diaSemana = hoje.getDay()
  const list = (d.data||[]).filter(a => {
    const dia = a.broadcast?.day_of_week
    const dias = {Monday:1,Tuesday:2,Wednesday:3,Thursday:4,Friday:5,Saturday:6,Sunday:0}
    return dias[dia] === diaSemana
  }).slice(0, 5)
  if (!list.length) return

  const campos = list.map(a => ({
