...
async function handleInfo(slug) {
  ...
  // Extrai episódios — AnimeFire usa vários padrões
  const epSet = new Set()

  // Padrão 1: /animes/{slug}/{n}
  const re1 = new RegExp(`/animes/${slug}/(\\d+)`, 'g')
  let m
  while ((m = re1.exec(html)) !== null) epSet.add(parseInt(m[1]))

  // Padrão 2: data-ep="N" ou data-episode="N"
  for (const mm of html.matchAll(/data-ep(?:isode)?=["'](\d+)["']/g))
    epSet.add(parseInt(mm[1]))

  // Padrão 3: href="/video/{slug}/{n}"
  const re3 = new RegExp(`/video/${slug}/(\\d+)`, 'g')
  while ((m = re3.exec(html)) !== null) epSet.add(parseInt(m[1]))

  // Padrão 4: se página carregou mas não achou eps, assume pelo menos EP1
  // (acontece com títulos curtos ou sem lista visível)
  if (epSet.size === 0) epSet.add(1)

  const episodes   = [...epSet].sort((a, b) => a - b).map(ep => ({ ep }))
  const titleMatch = html.match(/<h1[^>]*>([^<]+)</) || html.match(/<title>([^<|–]+)/)
  const title      = titleMatch?.[1]?.trim() || slug

  return { slug, title, episodes, total: episodes.length, domain: AF }
}
