...
const openCastTV = (url) => {
  const direct = getDirectUrl(url)
  window.location.href = `webvideocast://play?url=${encodeURIComponent(direct)}`
  setTimeout(() => {
    if (!document.hidden)
...
const openADM = (url, fn) => {
  const direct = getDirectUrl(url)
  window.location.href = `adm://add?url=${encodeURIComponent(direct)}&filename=${encodeURIComponent(fn)}`
  setTimeout(() => window.open(direct, '_blank'), 900)
}
...
export default function WatchPage() {
  ...
  const playerMode = loadPrefs().playerMode || 'internal'
