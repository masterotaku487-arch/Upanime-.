...
const openMXPlayer = (url, title) => {
  ...
  // Passa headers necessarios via parametros do MX Player
  // MX Player aceita: S.headers_Referer e S.headers_User-Agent
  const referer = encodeURIComponent('https://animefire.plus')
  const ua = encodeURIComponent('Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36')
  
  // Tenta URL direta primeiro (sem proxy) para MX Player
  // Se a URL e do worker proxy, tenta extrair a URL real
  let directUrl = url
  if (url.includes('stream-cdn') || url.includes('workers.dev/proxy')) {
    // Pega o param url= se existir
    try {
      const u = new URL(url)
      const param = u.searchParams.get('url')
      if (param) directUrl = decodeURIComponent(param)
    } catch {}
  }
