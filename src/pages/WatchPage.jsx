...
const openMXPlayer = (url, title) => {
  // Tenta MX Player Pro primeiro, depois Free, depois MX Player Ad
  const packages = [
  ...
  // Intent com fallback para todos os packages
  window.location.href = `intent:${url}#Intent;package=com.mxtech.videoplayer.ad;S.title=${titleEnc};S.decode_uri=${urlEnc};end`
  setTimeout(() => {
    if (!document.hidden) {
      // Fallback: tenta o Pro
      window.location.href = `intent:${url}#Intent;package=com.mxtech.videoplayer.pro;S.title=${titleEnc};end`
    }
  }, 800)
  setTimeout(() => {
    if (!document.hidden) {
      // Ultimo fallback: abre a URL direto
