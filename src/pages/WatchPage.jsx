...
const openMXPlayer = (url, title) => {
  // Usa URL direta (sem proxy) para apps externos
  const directUrl = url.includes('/api/proxy?url=')
    ? decodeURIComponent(url.split('url=')[1])
    : url
  window.location.href = `intent:${directUrl}#Intent;package=com.mxtech.videoplayer.ad;S.title=${encodeURIComponent(title)};end`
  setTimeout(() => {
    // Fallback: abre Play Store se MX não estiver instalado
            ...
            {currentSrc && (
              <>
                <button className="ext-btn"
                  onClick={() => {
                    const dUrl = currentSrc.includes('/api/proxy?url=')
                      ? decodeURIComponent(currentSrc.split('url=')[1])
                      : currentSrc
                    window.open(`https://www.webvideocast.com/play?url=${encodeURIComponent(dUrl)}`, '_blank')
                  }}>
                  📡<span>Cast TV</span>
                </button>
