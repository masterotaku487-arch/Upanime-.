...
const SLUG_MAP = {
  // ── Slugs manuais — animes fora do padrão AnimeFire ─────────────────────
  ...
  40748: 'kimetsu-no-yaiba-mugen-ressha-hen',    // Demon Slayer Movie
  // Attack on Titan — AnimeFire usa o slug base sem temporada
  16498: 'shingeki-no-kyojin',                   // AoT T1
  25777: 'shingeki-no-kyojin',                   // AoT T2
  35760: 'shingeki-no-kyojin',                   // AoT T3
  38524: 'shingeki-no-kyojin',                   // AoT Final Season
  40028: 'shingeki-no-kyojin',                   // AoT Final Season Part 2
  51535: 'shingeki-no-kyojin',                   // AoT Final Season Part 3
  35122: 'shingeki-no-kyojin',                   // AoT The Movie / Final Arc
  // Outros com slug especial
  20: 'naruto',                                  // Naruto clássico
...
const buildSlugCandidates = (anime, dub = false) => {
  ...
  // Adiciona slugs base (sem sufixo) como candidatos extras no final
  // Ex: se temos "shingeki-no-kyojin-the-final-season", adiciona "shingeki-no-kyojin" também
  const extraBases = new Set()
  for (const base of bases) {
    // Remove sufixos de temporada comuns
    const stripped = base
    ...
    if (stripped && stripped !== base && stripped.length > 2) {
      extraBases.add(stripped)
    }
  }
  const extraCandidates = []
  for (const base of extraBases) {
    if (dub) {
      extraCandidates.push(base + '-dublado-todos-os-episodios')
      extraCandidates.push(base + '-dublado')
    } else {
      extraCandidates.push(base + '-todos-os-episodios')
    }
    extraCandidates.push(base)
  }

  return [...new Set([...withSeason, ...withoutSeason, ...extraCandidates])]
}
